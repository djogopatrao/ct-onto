...
EOF;
	...
	public function go() {
        ...
        foreach( $executor->configuration['prefix'] as $pre => $uri ) {
            $this->output_stream->out( sprintf("@prefix %s <%s>.",$pre,$uri) );
        }
...
class RdfReplace{
	...
	public function go() {
        ...
        $i=0;
        $state = 0;
        while( $i<strlen($this->rdf) ) {
            $char = substr( $this->rdf,$i,1 );
            $char2 = substr( $this->rdf,$i,2 );
            if ( $char2 == '\"{' && $state == 0 ) {
                $state = 1;
                $i+=2;
                $buffer = "";
            } else if ( $char2 == '}\"' and $state == 1 ) {
                $state = 0;
                $i+=2;
                $x .= $this->callback_quoted( array( $buffer, $buffer )  );
            } else if ( $state == 1 ) {
                $buffer .= $char;
                $i++;
                ...
                $i++;
                $buffer = "";
            } else if ( $char == '}' and $state == 2 ) {
                $state = 0;
                $i++;
                $x .= $this->callback( array( $buffer, $buffer ) );
            } else if ( $state == 2 ) {
                $buffer .= $char;
                $i++;
            } else if ( $state == 0 ) {
                ...
                throw new Exception("Unknown parser state=$state, parsed_chars[$i]=$char2");
            }
        }
